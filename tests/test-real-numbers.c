...
/* ------------------------------------------------------------------ */
  ...
  DOIT_FOR_UNSIGNED_EXACT_INTEGER(libc_time);
...
#define DOIT_FOR_THIS_NUMBER(STEM,OP,ROP)		    \
  {							    \
    auto	op   = mmux_## STEM ## _literal(OP);	    \
    auto	erop = mmux_## STEM ## _literal(ROP);	    \
    auto	rop1 = mmux_## STEM ##_ceil(op);	    \
    auto	rop2 = mmux_ctype_ceil(op);		    \
    EQUAL_UNARY(STEM, ceil, OP, erop, rop1);		    \
    EQUAL_UNARY(STEM, ceil, OP, erop, rop2);		    \
    dprintf(2," %s,", #STEM);				    \
  }
...
#define DOIT_FOR_FLONUM(STEM)				    \
  {							    \
    DOIT_FOR_THIS_NUMBER(STEM,	+0.123,	+1.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	-0.123,	-0.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	-0.456,	-0.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	+0.0,	+0.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	-0.0,	-0.0);		    \
  }
...
#define DOIT_FOR_FLONUMC(STEM)						\
  {									\
    DOIT_FOR_THIS_FLONUMC_NUMBER(STEM,	+0.123,	+0.456,	+1.0,	+1.0);	\
    DOIT_FOR_THIS_FLONUMC_NUMBER(STEM,	-0.123,	-0.456,	-0.0,	-0.0);	\
  }
...
#define DOIT_FOR_THIS_NUMBER(STEM,OP,ROP)		    \
  {							    \
    auto	op   = mmux_## STEM ## _literal(OP);	    \
    auto	erop = mmux_## STEM ## _literal(ROP);	    \
    auto	rop1 = mmux_## STEM ##_floor(op);	    \
    auto	rop2 = mmux_ctype_floor(op);		    \
    EQUAL_UNARY(STEM, floor, OP, erop, rop1);		    \
    EQUAL_UNARY(STEM, floor, OP, erop, rop2);		    \
    dprintf(2," %s,", #STEM);				    \
  }
...
#define DOIT_FOR_FLONUM(STEM)				    \
  {							    \
    DOIT_FOR_THIS_NUMBER(STEM,	+1.234,	+1.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	-1.123,	-2.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	+0.0,	+0.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	-0.0,	-0.0);		    \
  }
...
#define DOIT_FOR_FLONUMC(STEM)						\
  {									\
    DOIT_FOR_THIS_FLONUMC_NUMBER(STEM,	+1.234,	+5.678,	+1.0,	+5.0);	\
    DOIT_FOR_THIS_FLONUMC_NUMBER(STEM,	-1.234,	-5.678,	-2.0,	-6.0);	\
  }
...
#define DOIT_FOR_THIS_NUMBER(STEM,OP,ROP)		    \
  {							    \
    auto	op   = mmux_## STEM ## _literal(OP);	    \
    auto	erop = mmux_## STEM ## _literal(ROP);	    \
    auto	rop1 = mmux_## STEM ##_trunc(op);	    \
    auto	rop2 = mmux_ctype_trunc(op);		    \
    EQUAL_UNARY(STEM, trunc, OP, erop, rop1);		    \
    EQUAL_UNARY(STEM, trunc, OP, erop, rop2);		    \
    dprintf(2," %s,", #STEM);				    \
  }
...
#define DOIT_FOR_FLONUM(STEM)				    \
  {							    \
    DOIT_FOR_THIS_NUMBER(STEM,	+1.234,	+1.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	-1.123,	-1.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	+0.0,	+0.0);		    \
    DOIT_FOR_THIS_NUMBER(STEM,	-0.0,	-0.0);		    \
  }
...
#define DOIT_FOR_THIS_NUMBER(STEM,OP,ROP)		    \
  {							    \
    auto	op   = mmux_## STEM ## _literal(OP);	    \
    auto	erop = mmux_## STEM ## _literal(ROP);	    \
    auto	rop1 = mmux_## STEM ##_rint(op);	    \
    auto	rop2 = mmux_ctype_rint(op);		    \
...
#define DOIT_FOR_FLONUMC(STEM)						\
  {									\
    DOIT_FOR_THIS_FLONUMC_NUMBER(STEM,	+1.234,	+5.678,	+1.0,	+6.0);	\
    DOIT_FOR_THIS_FLONUMC_NUMBER(STEM,	-1.234,	-5.678,	-1.0,	-6.0);	\
  }
...
#define DOIT_FOR_THIS_NUMBER(STEM,OP,ROP)		    \
  {							    \
    auto	op   = mmux_## STEM ## _literal(OP);	    \
    auto	erop = mmux_## STEM ## _literal(ROP);	    \
    auto	rop1 = mmux_## STEM ##_nearbyint(op);	    \
    auto	rop2 = mmux_ctype_nearbyint(op);	    \
    EQUAL_UNARY(STEM, rint, OP, erop, rop1);		    \
    EQUAL_UNARY(STEM, rint, OP, erop, rop2);		    \
    dprintf(2," %s,", #STEM);				    \
  }
...
#define DOIT_FOR_THIS_NUMBER(STEM,OP,ROP)		    \
  {							    \
    auto	op   = mmux_## STEM ## _literal(OP);	    \
    auto	erop = mmux_## STEM ## _literal(ROP);	    \
    auto	rop1 = mmux_## STEM ##_round(op);	    \
    auto	rop2 = mmux_ctype_round(op);		    \
    EQUAL_UNARY(STEM, rint, OP, erop, rop1);		    \
    EQUAL_UNARY(STEM, rint, OP, erop, rop2);		    \
    dprintf(2," %s,", #STEM);				    \
  }
...
#define DOIT_FOR_THIS_NUMBER(STEM,OP,ROP)		    \
  {							    \
    auto	op   = mmux_## STEM ## _literal(OP);	    \
    auto	erop = mmux_## STEM ## _literal(ROP);	    \
    auto	rop1 = mmux_## STEM ##_roundeven(op);	    \
    auto	rop2 = mmux_ctype_roundeven(op);	    \
    EQUAL_UNARY(STEM, rint, OP, erop, rop1);		    \
    EQUAL_UNARY(STEM, rint, OP, erop, rop2);		    \
    dprintf(2," %s,", #STEM);				    \
  }
...
#undef  DOIT_FOR_THIS_NUMBER
#define DOIT_FOR_THIS_NUMBER(STEM,OP,EXPECTED_INTEGER_PART,EXPECTED_FRACTIONAL_PART)	\
  {											\
    auto		op   = mmux_## STEM ## _literal(OP);				\
    auto		ei   = mmux_## STEM ## _literal(EXPECTED_INTEGER_PART);		\
    auto		ef   = mmux_## STEM ## _literal(EXPECTED_FRACTIONAL_PART);	\
    mmux_##STEM##_t	integer_part_1;							\
    mmux_##STEM##_t	integer_part_2;							\
    auto		fractional_part_1 = mmux_## STEM ##_modf(op, &integer_part_1);	\
    auto		fractional_part_2 = mmux_ctype_modf(op, &integer_part_2);	\
    EQUAL_UNARY(STEM, modf_integer,    OP, ei, integer_part_1);				\
    EQUAL_UNARY(STEM, modf_fractional, OP, ef, fractional_part_1);			\
    EQUAL_UNARY(STEM, modf_integer,    OP, ei, integer_part_2);				\
    EQUAL_UNARY(STEM, modf_fractional, OP, ef, fractional_part_2);			\
    dprintf(2," %s,", #STEM);								\
  }
...
#define DOIT_FOR_FLONUM(STEM)					\
  {								\
    DOIT_FOR_THIS_NUMBER(STEM,	+1.234,	+1.0,	+0.234);	\
    DOIT_FOR_THIS_NUMBER(STEM,	-1.234,	-1.0,	-0.234);	\
    DOIT_FOR_THIS_NUMBER(STEM,	+0.0,	+0.0,	+0.0);		\
    DOIT_FOR_THIS_NUMBER(STEM,	-0.0,	-0.0,	-0.0);		\
  }
