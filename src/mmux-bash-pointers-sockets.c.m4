...

/** --------------------------------------------------------------------
 ** Sockets: creation, shutdown, pair.
 ** ----------------------------------------------------------------- */
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_socket]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(protocol,			4);
  {
    ...
    MMUX_LIBC_FUNCALL(mmux_libc_socket(&sock, namespace, style, protocol));
    {
      mmux_bash_rv_t	brv = mmux_libc_fd_bind_to_bash_variable(sock_varname, sock, MMUX_BASH_BUILTIN_STRING_NAME);

      if (MMUX_SUCCESS != brv) {
	mmux_libc_close(sock);
      }
      return brv;
    }
  }
...
{
  mmux_libc_network_socket_t	sock;
  mmux_sint_t			shut_how;

  MMUX_BASH_PARSE_BUILTIN_ARGNUM_LIBC_FD(sock,	1);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(shut_how,	2);
  {
    MMUX_LIBC_FUNCALL(mmux_libc_shutdown(sock, shut_how));
    return MMUX_SUCCESS;
  }
}
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
    [[[(3 == argc)]]],
    [[["MMUX_BASH_BUILTIN_IDENTIFIER SOCK SHUT_HOW"]]])
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_socketpair]]])
{
  ...
  mmux_sint_t	pf_namespace, sock_style, ipproto;

  MMUX_BASH_PARSE_BUILTIN_ARGNUM_BASH_PARM(sock_varname1,	1);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_BASH_PARM(sock_varname2,	2);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(pf_namespace,		3);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(sock_style,		4);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(ipproto,			5);
  {
    mmux_libc_network_socket_t	sock1, sock2;

    MMUX_LIBC_FUNCALL(mmux_libc_socketpair(&sock1, &sock2, pf_namespace, sock_style, ipproto));
    {
      mmux_bash_rv_t	brv;

      brv = mmux_libc_fd_bind_to_bash_variable(sock_varname1, sock1, MMUX_BASH_BUILTIN_STRING_NAME);
      if (MMUX_SUCCESS != brv) { goto error_binding_variables; }

      brv = mmux_libc_fd_bind_to_bash_variable(sock_varname2, sock2, MMUX_BASH_BUILTIN_STRING_NAME);
      if (MMUX_SUCCESS != brv) { goto error_binding_variables; }

      return brv;

    error_binding_variables:
      mmux_libc_close(sock1);
      mmux_libc_close(sock2);
      return brv;
    }
  }
}
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_connect]]])
{
  mmux_libc_network_socket_t	sock;
  mmux_libc_sockaddr_ptr_t	sockaddr_pointer;
  mmux_socklen_t		sockaddr_size;

  MMUX_BASH_PARSE_BUILTIN_ARGNUM_LIBC_FD(sock,				1);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_TYPED_POINTER(sockaddr_pointer,	2);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SOCKLEN(sockaddr_size,			3);
  {
    MMUX_LIBC_FUNCALL(mmux_libc_connect(sock, sockaddr_pointer, sockaddr_size));
    return MMUX_SUCCESS;
  }
}
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
    [[[(4 == argc)]]],
    [[["MMUX_BASH_BUILTIN_IDENTIFIER SOCK SOCKADDR_POINTER SOCKADDR_SIZE"]]])
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_getpeername]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_BASH_PARM(addr_length_var,	3);
  {
    ...
    MMUX_LIBC_FUNCALL(mmux_libc_getpeername(sock, &sockaddr, &sockaddr_size));
    {
      mmux_libc_sockaddr_ptr_t	sockaddr_pointer;

      MMUX_LIBC_FUNCALL(mmux_libc_calloc(&sockaddr_pointer, 1, sockaddr_size));
      mmux_libc_memcpy(sockaddr_pointer, &sockaddr, sockaddr_size);
      {
	mmux_bash_rv_t	brv;
	...
	brv = mmux_socklen_bind_to_bash_variable(addr_length_var, sockaddr_size, MMUX_BASH_BUILTIN_STRING_NAME);
	if (MMUX_SUCCESS != brv) { goto error_binding_variables; }
...
{
  mmux_libc_network_socket_t	sock;
  mmux_asciizcp_t		sockaddr_pointer_var;
  mmux_asciizcp_t		sockaddr_size_var;

  MMUX_BASH_PARSE_BUILTIN_ARGNUM_LIBC_FD(sock,				1);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_BASH_PARM(sockaddr_pointer_var,	2);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_BASH_PARM(sockaddr_size_var,		3);
  {
    mmux_libc_sockaddr_t	sockaddr;
    mmux_socklen_t		sockaddr_size = sizeof(mmux_libc_sockaddr_t);

    MMUX_LIBC_FUNCALL(mmux_libc_getsockname(sock, &sockaddr, &sockaddr_size));
    {
      mmux_libc_sockaddr_ptr_t	sockaddr_pointer;
	...
	brv = mmux_pointer_bind_to_bash_variable(sockaddr_pointer_var, sockaddr_pointer, MMUX_BASH_BUILTIN_STRING_NAME);
	if (MMUX_SUCCESS != brv) { goto error_binding_variables; }

	brv = mmux_socklen_bind_to_bash_variable(sockaddr_size_var, sockaddr_size, MMUX_BASH_BUILTIN_STRING_NAME);
	if (MMUX_SUCCESS != brv) { goto error_binding_variables; }
...
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
    [[[(4 == argc)]]],
    [[["MMUX_BASH_BUILTIN_IDENTIFIER SOCKFD SOCKADDR_POINTER_VAR SOCKADDR_SIZE_VAR"]]])
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_bind]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SOCKLEN(sockaddr_size,			3);
  {
    MMUX_LIBC_FUNCALL(mmux_libc_bind(sock, sockaddr_pointer, sockaddr_size));
    return MMUX_SUCCESS;
  }
}
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_listen]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_LIBC_FD(sock,				1);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_UINT(pending_connections_queue_length,	2);
  {
    MMUX_LIBC_FUNCALL(mmux_libc_listen(sock, pending_connections_queue_length));
    return MMUX_SUCCESS;
  }
}
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_accept]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_LIBC_FD(server_sock,				3);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_TYPED_POINTER(sockaddr_pointer,		4);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SOCKLEN(sockaddr_size,				5);
  {
    mmux_libc_network_socket_t	connected_sock;

    /* Upon calling: "sockaddr_size" must be set to the number of bytes allocated for
     * the address referenced by "sockaddr_pointer".
     *
     * Upon successfully  returning: "sockaddr_size" is  reset to the actual  size of
     * the address.
     */
    MMUX_LIBC_FUNCALL(mmux_libc_accept(&connected_sock, sockaddr_pointer, &sockaddr_size, server_sock));
    {
      mmux_bash_rv_t	brv;

      brv = mmux_libc_fd_bind_to_bash_variable(connected_sock_varname, connected_sock, MMUX_BASH_BUILTIN_STRING_NAME);
      if (MMUX_SUCCESS != brv) { goto error_binding_variables; }

      brv = mmux_socklen_bind_to_bash_variable(connected_sockaddr_length_varname, sockaddr_size, MMUX_BASH_BUILTIN_STRING_NAME);
      if (MMUX_SUCCESS != brv) { goto error_binding_variables; }

      return brv;

    error_binding_variables:
      mmux_libc_close(connected_sock);
      return brv;
    }
  }
}
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_accept4]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(flags,					6);
  {
     ...
     * the address.
     */
    MMUX_LIBC_FUNCALL(mmux_libc_accept4(&connected_sock, sockaddr_pointer, &sockaddr_size, server_sock, flags));
    {
      mmux_bash_rv_t	brv;
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_send]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_LIBC_FD(sock,					2);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_POINTER(bufptr,				3);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_USIZE(buflen,					4);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(flags,					5);
  {
    mmux_usize_t	number_of_bytes_sent;

    MMUX_LIBC_FUNCALL(mmux_libc_send(&number_of_bytes_sent, sock, bufptr, buflen, flags));
    return mmux_usize_bind_to_bash_variable(number_of_bytes_sent_varname, number_of_bytes_sent, MMUX_BASH_BUILTIN_STRING_NAME);
  }
}
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
    [[[(6 == argc)]]],
    [[["MMUX_BASH_BUILTIN_IDENTIFIER NUMBER_OF_BYTES_SENT_VAR SOCK BUFPTR BUFLEN FLAGS"]]])
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_recv]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(flags,					5);
  {
    mmux_usize_t	number_of_bytes_received;

    MMUX_LIBC_FUNCALL(mmux_libc_recv(&number_of_bytes_received, sock, bufptr, buflen, flags));
    return mmux_usize_bind_to_bash_variable(number_of_bytes_received_varname, number_of_bytes_received, MMUX_BASH_BUILTIN_STRING_NAME);
  }
}
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
    [[[(6 == argc)]]],
    [[["MMUX_BASH_BUILTIN_IDENTIFIER NUMBER_OF_BYTES_RECEIVED_VAR SOCK BUFPTR BUFLEN FLAGS"]]])
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_sendto]]])
{
  ...
  mmux_usize_t			buflen;
  mmux_sint_t			flags;
  mmux_libc_sockaddr_ptr_t	destinatin_sockaddr_pointer;
  mmux_socklen_t		destinatin_sockaddr_size;
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_USIZE(buflen,					4);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(flags,					5);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_TYPED_POINTER(destinatin_sockaddr_pointer,	6);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SOCKLEN(destinatin_sockaddr_size,		7);
  {
    mmux_usize_t	number_of_bytes_sent;

    MMUX_LIBC_FUNCALL(mmux_libc_sendto(&number_of_bytes_sent, sock, bufptr, buflen, flags,
				       destinatin_sockaddr_pointer, destinatin_sockaddr_size));
    return mmux_usize_bind_to_bash_variable(number_of_bytes_sent_varname, number_of_bytes_sent, MMUX_BASH_BUILTIN_STRING_NAME);
  }
}
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
    [[[(8 == argc)]]],
    [[["MMUX_BASH_BUILTIN_IDENTIFIER NUMBER_OF_BYTES_SENT_VAR SOCK BUFPTR BUFLEN FLAGS DESTINATIN_SOCKADDR_POINTER DESTINATIN_SOCKADDR_SIZE"]]])
...
{
  mmux_asciizcp_t		number_of_bytes_received_varname;
  mmux_asciizcp_t		sender_sockaddr_size_varname;
  mmux_libc_network_socket_t	sock;
  mmux_pointer_t		bufptr;
  mmux_usize_t			buflen;
  mmux_sint_t			flags;
  mmux_libc_sockaddr_ptr_t	sender_sockaddr_pointer;
  mmux_socklen_t		sender_sockaddr_size;

  MMUX_BASH_PARSE_BUILTIN_ARGNUM_BASH_PARM(number_of_bytes_received_varname,	1);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_BASH_PARM(sender_sockaddr_size_varname,	2);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_LIBC_FD(sock,					3);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_POINTER(bufptr,				4);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_USIZE(buflen,					5);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(flags,					6);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_TYPED_POINTER(sender_sockaddr_pointer,		7);
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SOCKLEN(sender_sockaddr_size,			8);
  {
    /* Upon entering  the call "sender_sockaddr_size"  must be  set to the  number of
     * bytes allocated to contain the sender address.
     *
     * Upon successfully returning  from the call "sender_sockaddr_size"  is reset to
     * the actual number of bytes required to contain the sender address.
     *
     * The arguments  "sender_sockaddr_pointer" and "&sender_sockaddr_size"  can be
     * NULL if we are not interested in retrieving the sender address.
     */
    mmux_usize_t	number_of_bytes_received;

    MMUX_LIBC_FUNCALL(mmux_libc_recvfrom(&number_of_bytes_received, sender_sockaddr_pointer, &sender_sockaddr_size,
					 sock, bufptr, buflen, flags));
    {
      mmux_bash_rv_t	brv;

      brv =  mmux_usize_bind_to_bash_variable(number_of_bytes_received_varname, number_of_bytes_received, MMUX_BASH_BUILTIN_STRING_NAME);
      if (MMUX_SUCCESS != brv) { goto error_binding_variables; }

      brv = mmux_socklen_bind_to_bash_variable(sender_sockaddr_size_varname, sender_sockaddr_size, MMUX_BASH_BUILTIN_STRING_NAME);
      if (MMUX_SUCCESS != brv) { goto error_binding_variables; }
...
MMUX_BASH_DEFINE_TYPICAL_BUILTIN_FUNCTION([[[MMUX_BASH_BUILTIN_IDENTIFIER]]],
    [[[(9 == argc)]]],
    [[["MMUX_BASH_BUILTIN_IDENTIFIER NUMBER_OF_BYTES_RECEIVED_VAR CONNECTED_SOCKADDR_SIZE_VAR SOCK BUFPTR BUFLEN FLAGS SOCKADDR_POINTER ALLOCATED_SOCKADDR_SIZE"]]])
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_getsockopt]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(optname,				4);
  {
	...
	MMUX_LIBC_FUNCALL(mmux_libc_getsockopt(&optval, &optlen, sock, level, optname));
	...
	MMUX_LIBC_FUNCALL(mmux_libc_getsockopt(&optval, &optlen, sock, level, optname));
	{
	  ...
	  mmux_sint_t				l_onoff, l_linger;
	  mmux_bash_rv_t			brv;

	  mmux_libc_l_onoff_ref (&l_onoff,  &optval);
	  mmux_libc_l_linger_ref(&l_linger, &optval);

	  brv = mmux_bash_assoc_array_find_or_make_mutable(&assoc_array_variable, option_value_varname, MMUX_BASH_BUILTIN_STRING_NAME);
	  if (MMUX_SUCCESS != brv) { return brv; }

	  brv = mmux_bash_assoc_array_bind(assoc_array_variable, "ONOFF", ((l_onoff)? "1" : "0"), MMUX_BASH_BUILTIN_STRING_NAME);
	  if (MMUX_SUCCESS != brv) { return brv; }
	  {
	    mmux_sint_t	requested_size = mmux_sint_sprint_size(l_linger);
	    ...
	    if (-1 == requested_size) {
	      ...
	      rv = mmux_sint_sprint(timeout_period_in_seconds, requested_size, l_linger);
	  ...
	  error_converting_l_linger:
	    mmux_libc_dprintfer("%s: error: failure while converting l_linger to string\n", MMUX_BASH_BUILTIN_STRING_NAME);
	    return MMUX_FAILURE;
	  }
	}
      }
...
MMUX_BASH_BUILTIN_MAIN([[[mmux_libc_setsockopt]]])
{
  ...
  MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(optname,	3);
  {
	...
	MMUX_BASH_PARSE_BUILTIN_ARGNUM_SINT(optval,	4);
	{
	  ...
	  MMUX_LIBC_FUNCALL(mmux_libc_setsockopt(sock, level, optname, &optval, optlen));
	  return MMUX_SUCCESS;
	}
      }
	...
	MMUX_BASH_PARSE_BUILTIN_ARGNUM_BASH_PARM(option_value_array_varname,	4);
	{
	  ...
	  /* Retrieve the value of the key "ONOFF". */
	  {
	    ...
	    if (MMUX_SUCCESS != brv) { return brv; }
	    mmux_libc_l_onoff_set(&optval, ((0 == strcmp("0",assoc_array_value))? 0 : 1));
	  }
	  ...
	  /* Retrieve the value of the key "LINGER". */
	  {
	    ...
	    if (false == mmux_sint_parse(&timeout_period_in_seconds, assoc_array_value, MMUX_BASH_BUILTIN_STRING_NAME)) {
	      mmux_libc_l_linger_set(&optval, timeout_period_in_seconds);
	    } else {
	      return MMUX_FAILURE;
	    }
	  }
